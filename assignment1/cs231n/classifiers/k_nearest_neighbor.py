...
class KNearestNeighbor(object):
  ...
  def compute_distances_one_loop(self, X):
    ...
    dists = np.zeros((num_test, num_train))
    for i in range(num_test):
      # dist = np.sqrt(np.sum(np.power(np.subtract(self.X_train, X[i]),2) ,1))
  ...
  def compute_distances_no_loops(self, X):
    ...
    minus2xy = np.dot(X, np.transpose(self.X_train))*(-2)
    xpower2 = np.dot(np.power(X, 2), np.ones((img_len, num_train)))
    ypower2 = np.dot(np.ones((num_test, img_len)), np.transpose(np.power(self.X_train,2)))
